...
const areaData = {
  '전두엽': { streak: 7, complete: 42, challenge: 18, play: 60, good: '전두엽', goodPct: 82, bad: '소뇌',   badPct: 38, prevScore: 40, thisScore: 60, scores: [55,60,58,70,65,75,60], prevScores: [40,42,38,45,50,42,40] },
  '소뇌':   { streak: 3, complete: 28, challenge: 10, play: 38, good: '소뇌',   goodPct: 65, bad: '측두엽', badPct: 44, prevScore: 35, thisScore: 42, scores: [40,38,42,45,50,44,42], prevScores: [30,32,28,35,38,34,35] },
  '해마':   { streak: 5, complete: 35, challenge: 14, play: 49, good: '해마',   goodPct: 75, bad: '두정엽', badPct: 50, prevScore: 50, thisScore: 65, scores: [58,62,60,68,65,72,65], prevScores: [48,50,46,52,55,50,50] },
  '후두엽': { streak: 1, complete: 15, challenge: 6,  play: 21, good: '후두엽', goodPct: 60, bad: '해마',   badPct: 48, prevScore: 38, thisScore: 42, scores: [40,38,44,42,46,44,42], prevScores: [36,34,38,36,40,38,38] },
  '두정엽': { streak: 2, complete: 20, challenge: 8,  play: 28, good: '두정엽', goodPct: 70, bad: '전두엽', badPct: 55, prevScore: 45, thisScore: 50, scores: [48,50,52,55,50,54,50], prevScores: [42,44,40,46,48,44,45] },
}
...
export default function ProgressPage() {
  ...
  return (
    <>
        ...
        <main className="main">
          ...
          <div className="stats-grid">
            <div className="stat-card blue">
              ...
              <div className="stat-value">{d.complete}<span>H</span></div>
            </div>
            <div className="stat-card green">
              <span className="stat-tag green">도전</span>
              <div className="stat-value">{d.challenge}<span>H</span></div>
            </div>
            <div className="stat-card orange">
              <span className="stat-tag orange">플레이</span>
              <div className="stat-value">{d.play}<span>회</span></div>
            </div>
          </div>
          ...
          <div className="insight-grid">
            <div className="insight-card">
              ...
              <div className="insight-bar-wrap">
                <div className="insight-bar good" style={{width: d.goodPct + '%'}}></div>
              </div>
              <div className="insight-desc">정답률 <strong>{d.goodPct}%</strong> · 최고 기록 🏆</div>
            </div>
            ...
            <div className="insight-card">
              ...
              <div className="insight-bar-wrap">
                <div className="insight-bar bad" style={{width: d.badPct + '%'}}></div>
              </div>
              <div className="insight-desc">정답률 <strong>{d.badPct}%</strong> · 더 연습해봐요 💪</div>
            </div>
            ...
            <div className="insight-card">
              ...
              <div style={{display:'flex', alignItems:'center', gap:'10px', margin:'6px 0'}}>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:'10px', color:'var(--text-light)', marginBottom:'2px'}}>지난주</div>
                  <div className="compare-num" style={{fontSize:'22px'}}>{d.prevScore}</div>
                </div>
                <div style={{fontFamily:'Jua,sans-serif', fontSize:'18px', color: diff >= 0 ? 'var(--green)' : 'var(--danger)'}}>
                  {diff >= 0 ? `↑ +${diff}` : `↓ ${diff}`}
                </div>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:'10px', color:'var(--text-light)', marginBottom:'2px'}}>이번주</div>
                  <div className="compare-num" style={{fontSize:'22px', color: diff >= 0 ? 'var(--green)' : 'var(--danger)'}}>{d.thisScore}</div>
                </div>
              </div>
              <div className="compare-sub">
                {diff >= 0 ? `이번 주가 ${diff}점 올랐어요 📈` : `이번 주가 ${Math.abs(diff)}점 내렸어요 📉`}
              </div>
            </div>
          </div>
            ...
            <div className="chart-header">
              ...
              <div className="chart-legend">
                <div className="legend-item">
                  <div className="legend-dot" style={{background:'var(--blue-dark)'}}></div>
                  이번 주
                </div>
                <div className="legend-item">
                  <div className="legend-dot" style={{background:'var(--blue-pale)'}}></div>
                  지난 주
                </div>
              </div>
            </div>
                ...
                return (
                  <div key={day} className="bar-col">
                    <div style={{display:'flex', alignItems:'flex-end', gap:'3px', height:'90px'}}>
                      <div className="bar prev" style={{height: prevH + 'px', flex:1}}></div>
                      <div className="bar this" style={{height: thisH + 'px', flex:1}}></div>
                    </div>
      ...
      {/* 설정 모달 */}
      {modalOpen && (
        <div className="s-modal open" onClick={(e) => { if (e.target.classList.contains('s-modal')) setModalOpen(false) }}>
          <div className="s-modal-content">
            ...
            <div className="s-tab-bar">
              {['game','sound','account'].map(tab => (
                <button key={tab} className={`s-tab-btn ${settingTab === tab ? 'active' : ''}`} onClick={() => setSettingTab(tab)}>
              ...
              <div className="s-tab-panel active">
                ...
                <div className="s-row">
                  ...
                  <label className="s-toggle">
                    <input type="checkbox" checked={settings.hint}
                ...
                <div className="s-row">
                  ...
                  <label className="s-toggle">
                    <input type="checkbox" checked={settings.timer}
            ...
            {settingTab === 'sound' && (
              <div className="s-tab-panel active">
                ...
                <div className="s-row">
                  ...
                  <label className="s-toggle">
                    <input type="checkbox" checked={settings.bgm}
                ...
                <div className="s-row s-row-col">
                  ...
                  <div className="s-slider-wrap">
                    ...
                    <input type="range" className="s-slider" min="0" max="100" value={settings.volume}
            ...
            <div className="s-modal-footer">
              <button className="s-save-btn" onClick={() => {
